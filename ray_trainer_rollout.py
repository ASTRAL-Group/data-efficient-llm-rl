...
import shutil 
from verl.trainer.ppo.selection_algorithms import *
import time
...
SELECTION_STRATEGIES = {
    "all": selection_all,
    "average_rewards": selection_by_average_rewards,
    # "level": selection_by_level,
    # "responses_similarity": selection_by_responses_similarity,
    "responses_ppl": selection_by_responses_ppl,
    "responses_entropy": selection_by_responses_entropy,
}

SELECTION_WITHOUT_ROLLOUT_STRATEGIES = ["prompt_ppl_without_rollout",
                                        "question_ppl_without_rollout"]
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        rollout_shuffle = True if self.config.data.selection_strategy == "random" else False
        self.train_dataloader = DataLoader(dataset=self.train_dataset,
                                           batch_size=train_batch_size,
                                           shuffle=rollout_shuffle, #yifan: note here
                                           drop_last=True,
    ...
    def select_indices_prompt_question_ppl(self, selection_dataloader, num_selected_samples=None, criterion='max'):
        all_ppl = []
        for _, batch_dict in tqdm(enumerate(selection_dataloader), desc="Calculating Prompt/Question PPL"):
            batch = DataProto.from_single_dict(batch_dict)
            batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'])

            with torch.no_grad():
                outputs = self.actor_rollout_wg.compute_prompt_log_prob(batch)
                log_probs = outputs.batch['old_log_probs']  
                attention_mask = batch.batch['attention_mask'][:, 1:] 
                attention_mask = attention_mask.float()
                log_probs = log_probs * attention_mask  
                avg_log_prob = log_probs.sum(dim=1) / attention_mask.sum(dim=1)  
                ppl = torch.exp(-avg_log_prob)

            all_ppl.append(ppl)
        
        all_ppl = torch.cat(all_ppl, dim=0) 
        assert num_selected_samples is not None

        if criterion == 'min':
            sorted_indices = torch.argsort(all_ppl)[:num_selected_samples]
        else:
            sorted_indices = torch.argsort(all_ppl)[-num_selected_samples:]
    ...
    def fit(self):
        ...
        # set save freq
        assert self.config.data.selection_strategy in ['all', 'random']
        total_steps = len(self.train_dataloader)  * self.config.trainer.total_epochs

        selected_samples = {}
        selected_samples['dataset'] = self.config.data.train_files
        
        with tqdm(total=total_steps, desc="Rollout Progress") as pbar:
            for epoch in range(self.config.trainer.total_epochs):
                batches_all = []
                epoch_raw = {}
                epoch_metrics = {}
                max_batches = None
                if self.config.data.selection_strategy == "random":
                    max_batches = 5120 // self.config.data.train_batch_size
                    print("max_batches:", max_batches)
                solve_none = 0
                solve_all = 0
                selected_samples[epoch] = []

                with _timer('epoch', epoch_raw):
                    with _timer('rollout', epoch_raw):

                        for i, batch_dict in enumerate(tqdm(self.train_dataloader, desc="Rollout")):
                            # if i <= 11:
                            #     continue
                            ...
                            epoch_metrics['epoch/solve_none'] = solve_none
                            epoch_metrics['epoch/solve_all'] = solve_all


                            batches_all.append(batch)

                            with _timer('save_data', epoch_raw):
                                temp_batches_all = DataProto.concat(batches_all)
                                saved_data = defaultdict(dict)
                                for _, batch in enumerate(temp_batches_all):
                                    ...
                                    saved_data[key]['question'] = batch.non_tensor_batch['extra_info']['question']
                                    saved_data[key]['responses'] = saved_data[key].get('responses', []) + [response_texts]
                                    saved_data[key]['rewards'] = saved_data[key].get('rewards', []) + [batch.batch['token_level_scores'].sum(-1).item()]
